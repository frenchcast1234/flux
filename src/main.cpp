...
// ── globals (extern-declared in FluxOS.h / app headers) ───────────────────
int    g_launchApp = -1;
FluxOS gOS;

void setup() {
    // index 0 = HomeScreen (always home)
    gOS.addApp(new HomeScreen());    // 0
    // page 1 (grid indices 0-7 → app indices 1-8)
    ...
    // page 2 (grid indices 8-14 → app indices 9-15)
    ...
    gOS.begin(); // M5 init + boot logo + launch HomeScreen
}
